...
/**
 * Invalidates a role and empties its role set
 *
 * \return set<int> ids of all flows the role used to contain
 */
std::set<int> CRole::role_t::invalidate(CRoleMembership& role_membership) {
	set<int> flows(flow_set->begin(), flow_set->end());
	flow_set->clear();
	if (role_type == 'm' || role_type == 'p') { // check all sub_roles(multi-client and p2p roles only)
		for (set<role_t *>::iterator rs_iter = role_set_->begin();rs_iter != role_set_->end();rs_iter++) {
			...
			flows.insert(sub_role->flow_set->begin(), sub_role->flow_set->end());
			sub_role->flow_set->clear();
		}
	}
	role_num = 0;
	role_type = 'n';
