...
class CImport {
	...
	public:
		...
		void read_stream(std::istream & in_stream, unsigned int flowcount, const IPv6_addr & local_net = IPv6_addr(), const IPv6_addr & netmask = IPv6_addr());
	...
	private:
		...
		static bool invalidate_role(CRole::role_t& role_to_be_invalidated, vector<uint32_t>& role_conflict_vector, Subflowlist& active_flowlist, CRoleMembership& roleMembership); // helper function used in role conflict resolution
	...
	public:
		...
		void prepare_graphlet(CGraphlet * graphlet, CRoleMembership & roleMembership);
	...
	private:
		...
		uint32_t getUncompressedFileSize(std::ifstream & in_stream) const;
