...
CImport::CImport(const CFlowList & flowlist, const prefs_t & newprefs) :
		full_flowlist(flowlist), active_flowlist(full_flowlist.begin(),
				full_flowlist.end()), prefs(newprefs) {
 ...
 * \param newprefs Preferences settings
 */
CImport::CImport(const std::string & in_filename,
		const std::string & out_filename, const prefs_t & newprefs) :
		prefs(newprefs) {
...
void CImport::prepare_flowlist() {
	...
	int host_pairs = 0;
	for (CFlowList::iterator flowiterator = full_flowlist.begin();
			flowiterator != full_flowlist.end(); flowiterator++) { // Go through all flows
		...
		FlowHashKeyHostPair hostPairKey((flowiterator->localIP),
				(flowiterator->remoteIP));
		...
		if (FlowHashMapHostPairIterator == flowHmHostPair->end()) {
			...
			// An entry exists
			FlowHashMapHostPairIterator->second =
					FlowHashMapHostPairIterator->second + biflow_inc; // Update entry by biflow count
		}
	}
	...
	cout << "Done (qualified a total of " << unibiflow_count << " of "
			<< uniflow_count << " uniflows)";
	cout << " out of a total of " << full_flowlist.size()
			<< " flows. We have a total of " << host_pairs << " host pairs.\n";
	if (uIP_error > 0)
		cerr << "\n\nERROR: found " << uIP_error
				<< " unexpected new IP address(es) in " << __func__ << "()..\n\n";
...
bool CImport::set_localIP(IPv6_addr newLocalIP, int host_count) {
	...
	if (flowlistIterator_start != full_flowlist.end()) {
		...
		while (hc < (unsigned int) host_count
				&& flowlistIterator_end != active_flowlist.end()) {
		...
		if (flowlistIterator_end - flowlistIterator_start <= 0) {
			...
			cout << "This should be the same: "
					<< distance(flowlistIterator_start, flowlistIterator_end)
					<< "==" << getActiveFlowlistSize() << endl;
			cout << "i="
					<< distance(
							CFlowList::const_iterator(full_flowlist.begin()),
							active_flowlist.begin()) << ", j="
					<< distance(
							CFlowList::const_iterator(full_flowlist.begin()),
							active_flowlist.end()) << ", flow_count="
					<< getActiveFlowlistSize() << endl;
			return true;
		}
	} else {
 ...
 */
void CImport::prepare_graphlet(CGraphlet * graphlet,
	...
	CFlowFilter filter(active_flowlist, prefs); // For filtering by flow type and protocol
	...
	if (debug && prefs.summarize_srv_roles) {
		cout << " ** Found " << serverRole.get_role_count()
				<< " potential server roles.\n";
	}
	if (debug && prefs.summarize_clt_roles) {
		cout << " ** Found " << clientRole.get_role_count()
	...
	if (debug2) {
		...
		roleMembership.print_multi_members();
		cout << "First p2p role number: " << roleMembership.get_role_num()
				<< endl;
	}

	vector<uint32_t>& flow_server_role = serverRole.get_flow_role();
	if (prefs.summarize_multclt_roles) {
		clientRole.check_multiclient(flow_server_role, filter,
				prefs.summarize_srv_roles);
	}
	...
	if (prefs.summarize_p2p_roles) {
		...
		if (debug)
			cout << " ** Found " << p2pRole.get_cand_flow_num() - 1
					<< " potential p2p flows.\n";
	...
	for (unsigned int j = 0; j < getActiveFlowlistSize(); j++) {
		...
		if (flow_client_role[j] == 0 && flow_server_role[j] == 0
				&& flow_p2p_role[j] == 0) {
			single_flow_rolenum[j] = roleMembership.add_single_flow(
					active_flowlist[j].remoteIP, active_flowlist[j].dPkts);
		} else {
			single_flow_rolenum[j] = 0;
		}
	}
	...
	// (III) Process selected flows to graphlet data
	// ***************************************************
	//
	...
	while (i < getActiveFlowlistSize()) {
		...
		if ((flow_client_role[i] != 0) && (flow_server_role[i] != 0)
				&& prefs.summarize_clt_roles && prefs.summarize_srv_roles) {
			float clt_rating = clientRole.getRating(flow_client_role[i]);
			float srv_rating = serverRole.getRating(flow_server_role[i]);
			if (debug6) {
				cout << "INFO: ambiguous roles (client(" << flow_client_role[i]
						<< "," << clt_rating << ")+server("
						<< flow_server_role[i] << "," << srv_rating
						<< ")) for flow: " << i << endl;
				cout << ((clt_rating < srv_rating) ? "server" : "client")
						<< " wins" << endl;
			}
			CRole::role_t* client_role = clientRole.getRole(
					flow_client_role[i]);
			CRole::role_t* srv_role = serverRole.getRole(flow_server_role[i]);
			if (client_role == NULL || srv_role == NULL) { // can't resolve
				cerr
						<< "unable to resolve role conflict between client and server("
						<< i << ")" << endl;
				ambiguous_cs_roles_flows++;
			} else { // conflict resolution
				bool was_successful;
				// try to resolve conflict using role rating information. if not possible because of a limit(min number of flows per role), invalidate the role to free the flows
				if (srv_rating <= clt_rating) {
					was_successful = srv_role->removeFlow(i, active_flowlist,
							roleMembership);
					if (!was_successful) {
						was_successful = invalidate_role((*srv_role),
								flow_server_role, active_flowlist,
								roleMembership);
					}
				} else {
					was_successful = client_role->removeFlow(i, active_flowlist,
							roleMembership);
					if (!was_successful) {
						was_successful = invalidate_role((*client_role),
								flow_client_role, active_flowlist,
								roleMembership);
					}
				}
				if (!was_successful) {
		...
		} else if ((flow_client_role[i] != 0) && (flow_p2p_role[i] != 0)
				&& prefs.summarize_clt_roles && prefs.summarize_p2p_roles) {
			float clt_rating = clientRole.getRating(flow_client_role[i]);
			float p2p_rating = p2pRole.getRating(flow_p2p_role[i]);
			if (debug6) {
				cout << "INFO: ambiguous roles (client(" << flow_client_role[i]
						<< "," << clt_rating << ")+p2p(" << flow_p2p_role[i]
						<< "," << p2p_rating << ")) for flow: " << i << endl;
				cout << ((clt_rating < p2p_rating) ? "p2p" : "client")
						<< " wins" << endl;
			}
			CRole::role_t* client_role = clientRole.getRole(
					flow_client_role[i]);
			...
			if (client_role == NULL || p2p_role == NULL) { // can't resolve
				cerr
						<< "unable to resolve role conflict between client and p2p("
						<< i << ")" << endl;
				ambiguous_cp2p_roles_flows++;
			} else { // conflict resolution
				bool was_successful;
				// try to resolve conflict using role rating information. if not possible because of a limit(min number of flows per role), invalidate the role to free the flows
				if (p2p_rating <= clt_rating) {
					was_successful = p2p_role->removeFlow(i, active_flowlist,
							roleMembership);
					if (!was_successful) {
						was_successful = invalidate_role((*p2p_role),
								flow_p2p_role, active_flowlist, roleMembership);
					}
				} else {
					was_successful = client_role->removeFlow(i, active_flowlist,
							roleMembership);
		...
		} else if ((flow_server_role[i] != 0) && (flow_p2p_role[i] != 0)
				&& prefs.summarize_srv_roles && prefs.summarize_p2p_roles) {
			float srv_rating = serverRole.getRating(flow_server_role[i]);
			float p2p_rating = p2pRole.getRating(flow_p2p_role[i]);
			if (debug6) {
				cout << "INFO: ambiguous roles (server(" << flow_server_role[i]
						<< "," << srv_rating << ")+p2p(" << flow_p2p_role[i]
						<< "," << p2p_rating << ")) for flow: " << i << endl;
				cout << ((srv_rating < p2p_rating) ? "p2p" : "server")
						<< " wins" << endl;
			}
			CRole::role_t* srv_role = serverRole.getRole(flow_server_role[i]);
			CRole::role_t* p2p_role = p2pRole.getRole(flow_p2p_role[i]);
			if (srv_role == NULL || p2p_role == NULL) { // can't resolve
				cerr
						<< "unable to resolve role conflict between server and p2p("
						<< i << ")" << endl;
				ambiguous_sp2p_roles_flows++;
			} else { // conflict resolution
				bool was_successful;
				// try to resolve conflict using role rating information. if not possible because of a limit(min number of flows per role), invalidate the role to free the flows
				if (p2p_rating <= srv_rating) {
					was_successful = p2p_role->removeFlow(i, active_flowlist,
							roleMembership);
					if (!was_successful) {
						was_successful = invalidate_role((*p2p_role),
		...
		if (filter.filter_flow(i)) {
			filtered_flows++;
		} else if ((prefs.summarize_clt_roles && (flow_client_role[i] != 0))
				|| (prefs.summarize_srv_roles && (flow_server_role[i] != 0))
			...
			graphlet->add_single_flow(active_flowlist[i],
					single_flow_rolenum[i], i);
		}
	...
	if (ambiguous_cs_roles_flows > 0)
		cerr << "INFO: ambiguous roles (client+server) for "
				<< ambiguous_cs_roles_flows << " flows.\n";
	if (ambiguous_cp2p_roles_flows > 0)
		cerr << "INFO: ambiguous roles (client+p2p) for "
				<< ambiguous_cp2p_roles_flows << " flows.\n";
	if (ambiguous_sp2p_roles_flows > 0)
		cerr << "INFO: ambiguous roles (server+p2p) for "
				<< ambiguous_sp2p_roles_flows << " flows.\n";
	if (summarized_flows)
		cout << "Summarized flows: " << summarized_flows << endl;
	if (filtered_flows)
		cout << "Filtered flows: " << filtered_flows << " out of "
				<< getActiveFlowlistSize() << " flows." << endl;

	if (debug) {
		desummarizedRoles::const_iterator dri;
		cout << "desummarized roles:\t";
		for (dri = desummarizedRolesSet.begin();
				dri != desummarizedRolesSet.end(); dri++) {
			cout << (*dri) << ",";
		}
		cout << endl;
...
void CImport::cflow2hpg() {
	...
	try {
		...
		stringstream error;
		error << "Could not open output file: " << hpg_filename;
		throw error.str();
	}
...
/**
 * Invalidate a specific role. used in role conflict resolution
 *
 *	\param role_to_be_invalidated Role to be desummarized
 *	\param role_conflict_cector A set of desummarized roles
 *	\param active_flowlist List containing all currently active flows
 *	\param roleMembership Role membership object
 *
 *	\return bool true, if role invalidation was sucessful
 */
bool CImport::invalidate_role(CRole::role_t& role_to_be_invalidated,
		vector<uint32_t>& role_conflict_vector, Subflowlist& active_flowlist,
		CRoleMembership& roleMembership) {
	// invalidate role
	if (debug6) {
		cout << "invalidating role:" << endl;
		role_to_be_invalidated.print_role();
	}
	set<int> freed_flows = role_to_be_invalidated.invalidate(roleMembership);
	assert(freed_flows.size()>0);
	// remove references
	set<int>::const_iterator flow_id;
	for (set<int>::const_iterator flow_id = freed_flows.begin();
			flow_id != freed_flows.end(); ++flow_id) {
		assert((*flow_id)>=0 && (*flow_id)<(int)role_conflict_vector.size());
		role_conflict_vector[*flow_id] = 0;
	}
	assert(role_to_be_invalidated.flows==freed_flows.size());
	return true;
}
 ...
 */
void CImport::calculate_multi_summary_node_desummarizations(
		CRoleMembership & roleMembership) {
	...
	for (desummarizedRoles::const_iterator role_iter =
			desummarizedRolesSet.begin();
			role_iter != desummarizedRolesSet.end(); ++role_iter) {
	...
	CRoleMembership::multiSummaryNodeHashMap* m_s_nodes =
			roleMembership.get_hm_multiSummaryNode();
	for (CRoleMembership::multiSummaryNodeHashMap::const_iterator m_sum_iter =
			m_s_nodes->begin(); m_sum_iter != m_s_nodes->end(); ++m_sum_iter) {
		int multi_node_id = m_sum_iter->second->role_num;
		//cout << "candidate: " << multi_node_id << endl;
		set<int>::const_iterator mn_id_iter = multi_sum_node_ids.find(
				multi_node_id);
		if (mn_id_iter != multi_sum_node_ids.end()) { // multi-node should be desummarized
			...
			cout << "found!" << endl;
			const boost::array<uint16_t, 8> roles =
					m_sum_iter->first.getRoles();
			cout << util::bin2hexstring(&roles, 16) << endl;
			for (boost::array<uint16_t, 8>::const_iterator role_iter =
					roles.begin(); role_iter != roles.end(); ++role_iter) {
...
void CImport::get_hostMetadata() {
	...
	cout << "Input file " << in_filename << " contains " << maxnum_hosts
			<< " unique local hosts.\n";
	...
	while (it != active_flowlist.end()) {
		...
		if (it->localIP != hostMetadata[host_index].IP
				|| it == active_flowlist.begin()) {
			host_index++;
 ...
 */
Subflowlist CImport::get_flow(unsigned int flIndex,
		unsigned int flow_count) const {
	assert(full_flowlist.size() >= flIndex + flow_count);
	return Subflowlist(full_flowlist.begin() + flIndex,
			full_flowlist.begin() + flIndex + flow_count);
}
...
void CImport::print_flowlist(unsigned int linecount) {
	if (getActiveFlowlistSize() == 0) {
		...
		unsigned int maxcount =
				(linecount < getActiveFlowlistSize()) && (linecount != 0) ?
						linecount : getActiveFlowlistSize();
		copy(active_flowlist.begin(), active_flowlist.begin() + maxcount,
				ostream_iterator<cflow_t>(cout, "\n"));
	}
}
...
bool CImport::acceptForImport(const string & in_filename) {
	...
	for (importfilterIterator = inputfilters.begin();
			importfilterIterator != inputfilters.end();
			importfilterIterator++) {
 ...
 * \exception std::string Errormessage
 */
void CImport::write_file(std::string out_filename,
		const Subflowlist & subflowlist, bool appendIfExisting) {
	std::vector<GFilter *>::iterator filterIterator;

	if (inputfilters.empty())
		initInputfilters();

//	for (filterIterator = inputfilters.begin();
//			filterIterator != inputfilters.end(); filterIterator++) {
//		if ((*filterIterator)->acceptFileForWriting(out_filename)) {
//			try {
//				(*filterIterator)->write_file(out_filename, subflowlist,
//						appendIfExisting);
//			} catch (string & e) {
//				throw e;
//			}
 ...
 * \exception std::string Errormessage
 */
void CImport::write_file(std::string out_filename, const CFlowList & flowlist,
...
vector<std::string> CImport::getAllFormatNames() {
	...
	for (importfilterIterator = inputfilters.begin();
			importfilterIterator != inputfilters.end(); importfilterIterator++)
...
vector<std::string> CImport::getAllHumanReadablePatterns() {
	...
	for (importfilterIterator = inputfilters.begin();
			importfilterIterator != inputfilters.end(); importfilterIterator++)
		allTypeNames.push_back(
				(*importfilterIterator)->getHumanReadablePattern());
	return allTypeNames;
}
 ...
 */
void CImport::read_file(const IPv6_addr & local_net,
		const IPv6_addr & netmask) {
	...
	cout << "getting uncompressed file size\n";
	uncompressed_size = getUncompressedFileSize(cflow_compressed_inputstream);

	cout << "Check if this file can be a cflow_t-file\n";
	...
	cout << "opening gzifstream\n";
	gzifstream infs;
	infs.open(in_filename.c_str(), std::ios_base::in);

	read_stream(infs, flowcount, local_net, netmask);
	infs.close();
}
 ...
 */
uint32_t CImport::getUncompressedFileSize(ifstream & in_filestream) const {
...
void CImport::read_stream(std::istream & in_stream, unsigned int flowcount, const IPv6_addr & local_net, const IPv6_addr & netmask){
	if (inputfilters.empty())
			initInputfilters();
	...
	for (importfilterIterator = inputfilters.begin();
				...
				importfilterIterator++) {
			if ((*importfilterIterator)->acceptFileForReading(in_filename)) {
				cout << "File accepted for reading\n";
				try {
