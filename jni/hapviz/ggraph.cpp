 ...
 */
CGraphlet::CGraphlet(std::ostream & ostream, CRoleMembership & roleMembership) :
		outs(ostream) {
 ...
 */
CGraphlet::HashMapEdge CGraphlet::ipProtoToEdge(const IPv6_addr & ip,
 ...
 * \return HashMapEdge containing encoded fields
 */
CGraphlet::HashMapEdge CGraphlet::protoEportToEdge(const uint32_t proto,
		const uint64_t port) {
	HashMapEdge edge;
	edge.valueA.proto = proto;
 ...
 * \return HashMapEdge containing encoded fields
 */
CGraphlet::HashMapEdge CGraphlet::hNumRIpToEdge(const uint16_t hnum,
		const IPv6_addr & ip) {
	HashMapEdge edge;
 ...
 * \return HashMapEdge containing encoded fields
 */
CGraphlet::HashMapEdge CGraphlet::ePort1EPort2ToEdge(const uint64_t port1,
		const uint64_t port2) {
	HashMapEdge edge;
 ...
 * \return HashMapEdge containing encoded fields
 */
CGraphlet::HashMapEdge CGraphlet::ePortIpToEdge(const uint64_t port,
		const IPv6_addr & ip) {
	HashMapEdge edge;
	edge.valueB.port1 = port;
	edge.ip = ip;
 ...
 * \return HashMapEdge containing encoded fields
 */
CGraphlet::HashMapEdge CGraphlet::rBytesRPacketsToEdge(const uint32_t bytes,
 ...
 * \return HashMapEdge containing encoded fields
 */
CGraphlet::HashMapEdge CGraphlet::flowsPacketsToEdge(const uint32_t flows,
		const uint32_t packets) {
	HashMapEdge edge;
	edge.valueB.packets = packets;
 ...
 * \return HashMapEdge containing encoded fields
 */
CGraphlet::HashMapEdge CGraphlet::ePortRolnumClientsToEdge(const uint64_t port,
		const uint64_t rolnum_clients) {
	HashMapEdge edge;
 ...
 * \return HashMapEdge containing encoded fields
 */
CGraphlet::HashMapEdge CGraphlet::bytesFlowsToEdge(const uint32_t bytes,
 ...
 */
void CGraphlet::add_single_flow(const cflow_t & pflow, int role_num,
		int flow_idx) {
	totalbytes += pflow.dOctets; // Update total byte count per graphlet

	if (hap4nfsen) {
		nodeInfos->addNodeInfo(
				CSummaryNodeInfo::generate_node_info(local_ip, pflow),
				pflow.localIP.toNumericString());
	}
	...
	if (hap4nfsen) {
		nodeInfos->addNodeInfo(
				CSummaryNodeInfo::generate_node_info(proto, pflow),
				boost::lexical_cast<string>(static_cast<int>(prot)));
	}
	...
	if (iterIpProt == hm_localIp_prot->end()) {
		...
		(*hm_localIp_prot)[myIpProtkey] = ipProtoToEdge(
				(IPv6_addr) (pflow.localIP), prot);
	}
	...
	uint64_t localEportKey = getLocalEportKey(map_protonum(prot),
			pflow.localPort);
	uint64_t localEport = getLocalEport(pflow.flowtype, localEportKey);
	if (hap4nfsen) {
		nodeInfos->addNodeInfo(
				CSummaryNodeInfo::generate_node_info(local_port, pflow),
				boost::lexical_cast<string>(localEport & LOCAL_EPORT0_MASK));
	}
	...
	if (iterProtEport == hm_prot_localPort_11->end()) {
		(*hm_prot_localPort_11)[myProtEportkey] = protoEportToEdge(prot,
				localEport);
	}
	...
	uint64_t remoteEport = getRemoteEport(pflow.flowtype,
			map_protonum(pflow.prot), pflow.remoteIP.get24bitHash(),
			pflow.remotePort, (uint16_t) role_num);
	if (hap4nfsen) {
		nodeInfos->addNodeInfo(
				CSummaryNodeInfo::generate_node_info(remote_port, pflow),
				boost::lexical_cast<string>(remoteEport));
	}
	...
	if (iterEport2 == hm_localPort_remotePort_11->end()) {
		(*hm_localPort_remotePort_11)[myEport2key] = ePort1EPort2ToEdge(
				localEport, remoteEport);
	}
	...
	if (iterEport3 == hm_localPort_remotePortE->end()) {
		...
		(*hm_localPort_remotePortE)[myEport2key] = rBytesRPacketsToEdge(
				pflow.dOctets, pflow.dPkts);
	}

	// remotePort -- remoteIP  (key5/entry5)

	// Handle multi-connected remote hosts
	struct CRoleMembership::sumnode_t * sn = proleMembership->get_summaryNode(
			pflow.remoteIP);
	if (sn != NULL && sn->clients > 1) {
		...
		if (hap4nfsen) {
			nodeInfos->addNodeInfo(
					CSummaryNodeInfo::generate_node_info(remote_ip, pflow),
					boost::lexical_cast<string>((uint64_t) sumid));
		}
		iterEportIp = hm_remotePort_remoteIp_1n->find(myEportIpkey);
		if (iterEportIp == hm_remotePort_remoteIp_1n->end()) {
			(*hm_remotePort_remoteIp_1n)[myEportIpkey] =
					ePortRolnumClientsToEdge(remoteEport,
							getRolnumClients(sumid, sn->clients));
		}
		...
		if (iterEport3 == hm_remotePort_remoteIpE->end()) {
			...
			(*hm_remotePort_remoteIpE)[myEportIpkey] = flowsPacketsToEdge(flows,
					packets);
		} else {
		...
		if (iterEportIp == hm_remotePort_remoteIp_11->end()) {
			(*hm_remotePort_remoteIp_11)[myEportIpkey] = ePortIpToEdge(
					remoteEport, pflow.remoteIP);
		}
		// Use an extra hm to store flow count
		iterEport3 = hm_remotePort_remoteIpE->find(myEportIpkey);
		if (iterEport3 == hm_remotePort_remoteIpE->end()) {
			(*hm_remotePort_remoteIpE)[myEportIpkey] = flowsPacketsToEdge(1,
					pflow.dPkts);
		} else {
			// Link already exists: update link annotation
			HashMapEdge oldval = (HashMapEdge) iterEport3->second;
 ...
 */
void CGraphlet::add_generic_role(CRole::role_t & role,
		const CRole::role_t& parent_role, IPv6_addr lastIP,
		Subflowlist flow_list) {
	if (dbg) {
		cout << "adding generic role(num:" << role.role_num << ", parent num:"
				<< parent_role.role_num << ") with pattern:"
				<< util::graphletSummarizationToString(role.pattern)
				<< ", parent pattern:"
				<< util::graphletSummarizationToString(parent_role.pattern)
				<< " with " << role.flows << "flows" << endl;
	}
	...
	if (parent_role.role_type == 'm' || parent_role.role_type == 'p') {
		for (set<CRole::role_t*>::const_iterator role_id =
				parent_role.role_set_->begin();
				role_id != parent_role.role_set_->end(); role_id++) {
			flows.insert((*role_id)->flow_set->begin(),
					(*role_id)->flow_set->end());
		}
	}
	...
	if (hap4nfsen) {
		nodeInfos->addNodeInfo(
				CSummaryNodeInfo::generate_node_info(local_ip, ' ',
						role.pattern, flows, flow_list),
				flow->localIP.toNumericString());
	}

	// localIP -- protocol
	if (role.get_partition_association(local_ip, proto) != gpa_1_1) {
		cerr
				<< "ERROR: unsupported graphlet association between partitions local_ip and proto"
				<< endl;
		assert(false);
	}
	...
	if (hap4nfsen) {
		nodeInfos->addNodeInfo(
				CSummaryNodeInfo::generate_node_info(proto, role.role_type,
						role.pattern, flows, flow_list),
				boost::lexical_cast<string>(static_cast<int>(role.prot)));
	}
	iterIpProt = hm_localIp_prot->find(myIpProtkey);
	if (iterIpProt == hm_localIp_prot->end()) {
	...
	// protocol -- localPort
	graphlet_partition_association proto_lport_association =
			role.get_partition_association(proto, local_port);
	if (proto_lport_association != gpa_1_1
			&& proto_lport_association != gpa_1_n) {
		cerr
				<< "ERROR: unsupported graphlet association between partitions proto and local_port"
				<< endl;
		assert(false);
	}
	...
	for (set<int>::const_iterator flow_id = flows.begin();
			flow_id != flows.end(); flow_id++) {
		const cflow_t* flow = &(flow_list[*flow_id]);
		if (proto_lport_association == gpa_1_1) {
			uint64_t localEportKey = getLocalEportKey(map_protonum(flow->prot),
					flow->localPort);
			...
			iterProtEport = hm_prot_localPort_11->find(myProtEportkey);
			if (iterProtEport == hm_prot_localPort_11->end()) {
				(*hm_prot_localPort_11)[myProtEportkey] = protoEportToEdge(
						role.prot, localEport);
			}
			//cout<<localEport<<"-"<<localEportKey<<"-"<<(localEport&LOCAL_EPORT0_MASK)<<"-"<<(localEportKey&LOCAL_EPORT0_MASK)<<endl;
			if (hap4nfsen) {
				nodeInfos->addNodeInfo(
						CSummaryNodeInfo::generate_node_info(local_port,
								role.role_type, role.pattern, flows, flow_list),
						boost::lexical_cast<string>(
								localEport & LOCAL_EPORT0_MASK));
			}
			localEports.push_back(localEport);
			localEportKeys.push_back(localEportKey);
		} else { // must be gpa_1_n
			//util::printFlow(*flow);
			uint64_t localEport = getLocalEport(role.flowtype,
					role.getSubRoleId(local_port, parent_role), role.flows);
			...
			if (iterProtEport == hm_prot_localPort_1n->end()) {
				(*hm_prot_localPort_1n)[myProtEportkey] = protoEportToEdge(
						role.prot, localEport);
			}
			localEports.push_back(localEport);
			...
			if (hap4nfsen) {
				nodeInfos->addNodeInfo(
						CSummaryNodeInfo::generate_node_info(local_port,
								role.role_type, role.pattern, flows, flow_list),
						boost::lexical_cast<string>(localEport));
			}
	...
	graphlet_partition_association lport_rport_association =
			role.get_partition_association(local_port, remote_port);
	...
	for (set<int>::const_iterator flow_id = flows.begin();
			flow_id != flows.end(); flow_id++) {
		const cflow_t* flow = &(flow_list[*flow_id]);
		assert(localEports.size() > localEport_id);
		uint64_t localEport = localEports[localEport_id];
		uint64_t localEportKey = localEports[localEport_id]; // TODO: check
		uint64_t remoteEport;
		graphletHashKey myEport2key((uint32_t) 0, (uint32_t) 0);
		if (lport_rport_association == gpa_1_1) {
			if (role.get_partition_association(remote_port, remote_ip)
					== gpa_1_1) {
				uint16_t f_id =
						(is_fully_desummarized) ? (uint16_t) (*flow_id) : 0;
				remoteEport = getRemoteEport(role.flowtype,
						map_protonum(role.prot), flow->remoteIP.get24bitHash(),
						flow->remotePort, f_id);
			} else {
				remoteEport = getRemoteEport(role.flowtype, role.role_num,
						flow->remotePort);
			}
			...
			if (hap4nfsen) {
				nodeInfos->addNodeInfo(
						CSummaryNodeInfo::generate_node_info(remote_port,
								role.role_type, role.pattern, flows, flow_list),
						boost::lexical_cast<string>(remoteEport));
			}
		} else if (lport_rport_association == gpa_1_n) {
			remoteEport = getRemoteEport(role.flowtype,
					role.getSubRoleId(remote_port, parent_role), role.flows);
			myEport2key = graphletHashKey(localEportKey, remoteEport);
			iterEport2 = hm_localPort_remotePort_1n->find(myEport2key);
			if (iterEport2 == hm_localPort_remotePort_1n->end()) {
				(*hm_localPort_remotePort_1n)[myEport2key] = ePort1EPort2ToEdge(
						localEport, remoteEport);
			}
			if (!rp_sum_node_created) {
				if (hap4nfsen) {
					nodeInfos->addNodeInfo(
							CSummaryNodeInfo::generate_node_info(remote_port,
									role.role_type, role.pattern, flows,
									flow_list),
							boost::lexical_cast<string>(remoteEport));
				}
			...
			iterHnumIp = hm_hnum_remoteIp->find(myHnumIpkey);
			if (iterHnumIp == hm_hnum_remoteIp->end()) {
				(*hm_hnum_remoteIp)[myHnumIpkey] = hNumRIpToEdge(hostnum,
						role.remoteIP);
				current_hostnum = hostnum++; // May wrap around
			} else {
				// Fetch current host number from hash map
				current_hostnum =
						((HashMapEdge) iterHnumIp->second).valueA.hnum;
			}
			// Assemble remoteEport value
			uint16_t f_id = (is_fully_desummarized) ? (uint16_t) (*flow_id) : 0;
			remoteEport = getRemoteEport(role.flowtype,
					map_protonum(flow->prot), current_hostnum, flow->remotePort,
					f_id);
			...
			if (iterEport2 == hm_localPort_remotePort_n1->end()) {
				(*hm_localPort_remotePort_n1)[myEport2key] = ePort1EPort2ToEdge(
						localEport, remoteEport);
			}
			if (hap4nfsen) {
				nodeInfos->addNodeInfo(
			...
			if (iterEport2 == hm_localPort_remotePort_nn->end()) {
				(*hm_localPort_remotePort_nn)[myEport2key] = ePort1EPort2ToEdge(
						localEport, remoteEport);
			}
			if (!rp_sum_node_created) {
				if (hap4nfsen) {
		...
		if (iterEport3 == hm_localPort_remotePortE->end()) {
			...
			HashMapEdge oldval = (HashMapEdge) iterEport3->second;
			HashMapEdge newval = rBytesRPacketsToEdge(flow->dOctets,
					flow->dPkts);
		...
		localEport_id =
				(proto_lport_association == gpa_1_n) ?
						localEport_id : localEport_id + 1; // if associacion is gpa_1_n, vector contains only a single value
		if (lport_rport_association == gpa_n_n) {
	...
	// remotePort -- remoteIP
	graphlet_partition_association rport_rip_association =
			role.get_partition_association(remote_port, remote_ip);
	uint32_t remoteEport_id = 0;
	bool rip_sum_node_created = false;
	for (set<int>::iterator flow_id = flows.begin(); flow_id != flows.end();
			flow_id++) {
		const cflow_t* flow = &(flow_list[*flow_id]);
		assert(remoteEports.size() > remoteEport_id);
		uint64_t remoteEport = remoteEports[remoteEport_id];
		struct CRoleMembership::sumnode_t * sn =
				proleMembership->get_summaryNode(flow->remoteIP);
		if (rport_rip_association == gpa_1_n) {
			if (sn != NULL && sn->clients >= 1) {
				...
				if (hap4nfsen) {
					nodeInfos->addNodeInfo(
							CSummaryNodeInfo::generate_node_info(remote_ip,
									role.role_type, role.pattern, flows,
									flow_list), getSummaryNodeIdString(sn));
				}
				if (sn->clients == 1) {
					iterEportIp = hm_remotePort_remoteIp_11->find(myEportIpkey);
					if (iterEportIp == hm_remotePort_remoteIp_11->end()) {
						(*hm_remotePort_remoteIp_11)[myEportIpkey] =
								ePortIpToEdge(remoteEport, flow->remoteIP);
					}
				} else {
					iterEportIp = hm_remotePort_remoteIp_1n->find(myEportIpkey);
					if (iterEportIp == hm_remotePort_remoteIp_1n->end()) {
				...
				iterEport3 = hm_remotePort_remoteIpE->find(myEportIpkey);
				if (iterEport3 == hm_remotePort_remoteIpE->end()) {
					(*hm_remotePort_remoteIpE)[myEportIpkey] =
							flowsPacketsToEdge(1, flow->dPkts);
				} else {
					// Link already exists: update link annotation
					HashMapEdge oldval = (HashMapEdge) iterEport3->second;
				...
				if (iterEport3 == hm_remotePort_remoteIpE->end()) {
					(*hm_remotePort_remoteIpE)[myEportIpkey] =
							flowsPacketsToEdge(role.flows, role.packets);
				}
			}
			...
			if (sn != NULL && sn->role_map.size() >= 2) {
				if (hm_remotePort_remoteIp_11->find(myEportIpkey)
						== hm_remotePort_remoteIp_11->end()) {
			...
			for (set<IPv6_addr>::const_iterator it = role.rIP_set->begin();
					(rport_rip_association == gpa_n_n)
							&& (it != role.rIP_set->end()); it++) {
				struct CRoleMembership::sumnode_t * sn =
						proleMembership->get_summaryNode(*it);
				if (sn != NULL) {
					client_count--;
					// Add link to multi-summarynode
					graphletHashKey myEportIpkey(remoteEport, sn->role_num);
					if (sn->clients > 1) {
						iterEportIp = hm_remotePort_remoteIp_nn->find(
								myEportIpkey);
						if (iterEportIp == hm_remotePort_remoteIp_nn->end()) {
							(*hm_remotePort_remoteIp_nn)[myEportIpkey] =
									ePortRolnumClientsToEdge(remoteEport,
											getRolnumClients(sn->role_num,
													sn->clients));
						}
					} else {
						iterEportIp = hm_remotePort_remoteIp_n1->find(
								myEportIpkey);
						if (iterEportIp == hm_remotePort_remoteIp_n1->end()) {
							(*hm_remotePort_remoteIp_n1)[myEportIpkey] =
									ePortIpToEdge(remoteEport, *it);
						}
					}
					if (!rip_sum_node_created) {
						if (hap4nfsen) {
							nodeInfos->addNodeInfo(
									CSummaryNodeInfo::generate_node_info(
											remote_ip, role.role_type,
											role.pattern, flows, flow_list),
									getSummaryNodeIdString(sn));
						}
						rip_sum_node_created = true;
					}
					// Use an extra hm to store flow count
					...
					if (iterEport3 == hm_remotePort_remoteIpE->end()) {
						int packets = 0;
						int flows = sn->get_flowpacket_count(
								parent_role.role_num, packets);
						(*hm_remotePort_remoteIpE)[myEportIpkey] =
								flowsPacketsToEdge(flows, packets);
					} else {
					}
					int sn_packets = 0;
					int sn_flows = proleMembership->get_role_flowpacket_count(
							*it, parent_role.role_num, sn_packets);
					if (sn_flows == 0) {
						stringstream error;
						error << "Critical error in " << __FILE__ << ": #"
								<< __LINE__ << "\n\n";
						throw error.str();
					}
					role.flows -= sn_flows;
			...
			// Use node with single remote IP address
				...
				graphletHashKey myEportIpkey(remoteEport,
						(uint64_t) role.role_num);
				if (hap4nfsen) {
					nodeInfos->addNodeInfo(
							CSummaryNodeInfo::generate_node_info(remote_ip,
									role.role_type, role.pattern, flows,
				...
				if (iterEportIp == hm_remotePort_remoteIp_nn->end()) {
					...
					(*hm_remotePort_remoteIp_nn)[myEportIpkey] =
							ePortRolnumClientsToEdge(remoteEport,
									getRolnumClients(role.role_num,
											client_count));
				}
				// Use an extra hm to store flow count
				iterEport3 = hm_remotePort_remoteIpE->find(myEportIpkey);
 ...
 */
inline std::string CGraphlet::getSummaryNodeIdString(
		const CRoleMembership::sumnode_t* node) {
	hpg_field tmp_fld;
	tmp_fld.reset();
	tmp_fld.eightbytevalue.data = getRolnumClients(node->role_num,
			node->clients);
 ...
 */
inline uint64_t CGraphlet::getLocalEport(const uint8_t flow_type,
		const uint32_t role_nr, const uint32_t flow_count) {
	uint64_t localEportKey = (((uint64_t) role_nr & ROLE_NR_BIT_MASK)
			<< ROLE_SHIFT3) + (flow_count & FLOW_COUNT_BIT_MASK);
 ...
 * \return uint64_t 8 byte number with fields encoded according to the HPG specification
 */
inline uint64_t CGraphlet::getLocalEportKey(const uint8_t protocol,
 ...
 * \return uint64_t 8 byte number with fields encoded according to the HPG specification
 */
inline uint64_t CGraphlet::getRemoteEport(const uint8_t flowtype,
		const uint32_t role_num, const uint32_t remote_port) {
	...
	return color_shift + role_flowtype_shift
			+ (((uint64_t) role_num & ROLE_NR_BIT_MASK) << ROLE_SHIFT3)
			+ (remote_port & FLOW_COUNT_BIT_MASK);
}
 ...
 */
inline uint64_t CGraphlet::getRemoteEport(const uint8_t flowtype,
		const uint8_t protocol, const uint32_t hostnum,
		const uint16_t remote_port, const uint16_t flow_id) {
	...
	uint8_t color = flowtype2colorcode(flowtype);
	uint64_t color_shift = ((uint64_t) color << COLORTYPE_SHIFT);
	uint64_t flow_key = (((uint64_t) key_p1) << (sizeof(uint16_t) * 8))
			+ (((uint64_t) key_p2) << FLOWTYPE_SHIFT3);
	return color_shift + ((uint64_t) protocol << PROTCODE_SHIFT3)
			+ (((uint64_t) hostnum & HOSTNUM_BIT_MASK) << HOSTNUM_SHIFT)
			+ remote_port + flow_key;
}
 ...
 * \return uint64_t 8 byte number with fields encoded according to the HPG specification
 */
inline uint64_t CGraphlet::getRolnumClients(const uint32_t role_nr,
		const uint32_t client_count) {
	return (((uint64_t) role_nr & ROLE_NR_BIT_MASK) << ROLE_SHIFT3)
...
void CGraphlet::finalize_graphlet(int graphlet_nr) {
	...
	for (iterEport2 = hm_localPort_remotePort_11->begin();
			iterEport2 != hm_localPort_remotePort_11->end(); iterEport2++) {
		value[1].reset();
		value[2].reset();
		...
		if (iterEport3 != hm_localPort_remotePortE->end()) {
			...
			outs.write((char *) valueE, sizeof(valueE));
		} else {
	...
	value[0].eightbytevalue.data = (graphlet_nr << 4) + rank;
	for (iterEport2 = hm_localPort_remotePort_n1->begin();
			iterEport2 != hm_localPort_remotePort_n1->end(); iterEport2++) {
		value[1].reset();
		value[2].reset();
		HashMapEdge edge = (HashMapEdge) iterEport2->second;
		value[1].eightbytevalue.data = edge.valueB.port1; // localPort (eport)
	...
	value[0].eightbytevalue.data = (graphlet_nr << 4) + rank;
	for (iterEport2 = hm_localPort_remotePort_1n->begin();
			iterEport2 != hm_localPort_remotePort_1n->end(); iterEport2++) {
		value[1].reset();
		value[2].reset();
		HashMapEdge edge = (HashMapEdge) iterEport2->second;
		value[1].eightbytevalue.data = edge.valueB.port1; // localPort (eport)
	...
	value[0].eightbytevalue.data = (graphlet_nr << 4) + rank;
	for (iterEport2 = hm_localPort_remotePort_nn->begin();
			iterEport2 != hm_localPort_remotePort_nn->end(); iterEport2++) {
		value[1].reset();
		value[2].reset();
		HashMapEdge edge = (HashMapEdge) iterEport2->second;
		value[1].eightbytevalue.data = edge.valueB.port1; // localPort (eport)
	...
	for (iterEportIp = hm_remotePort_remoteIp_11->begin();
		...
		if (iterEport3 != hm_remotePort_remoteIpE->end()) {
			...
			if (flows == 0) {
				cerr
						<< "ERROR in CGraphlet::finalize_graphlet(): invalid flow (packet) count 0 ("
						<< packets << ")\n";
